...
cmd /c bitsadmin /transfer CygwinSetupExe /download /priority normal https://cygwin.com/setup-x86_64.exe %SystemDrive%\cygwin\cygwin-setup.exe

REM goto a temp directory
cd %SystemDrive%\windows\temp

REM run the installation
cmd /c %SystemDrive%\cygwin\cygwin-setup.exe -q -R %SystemDrive%\cygwin -P openssh,openssl,libsasl2,curl,cygrunsrv,wget,rebase,vim -s http://cygwin.mirrors.pair.com
