...
msg_status "Extracting kernel.."
SYS_BIN_DIR="$SUPPORT_DIR/tmp/sys_bin"
SYS_BIN_PKG="$MNT_BASE_SYSTEM/System/Installation/Packages/BaseSystemBinaries.pkg"
OLDPWD=$(pwd)

pkgutil --expand "$SYS_BIN_PKG" "$SYS_BIN_DIR"
cd $MNT_BASE_SYSTEM
pax -0jr < "$SYS_BIN_DIR/Payload" ./mach_kernel
cd "$OLDPWD"

msg_status "Transfering kernel extensions"
EXTENSIONS_DIR="$MNT_BASE_SYSTEM/System/Library/Extensions"
# Trailing slash is required because of the -R option given to cp
SUPPORT_EXTENSIONS_PATH="$SUPPORT_DIR/S_L_Extensions/"
[ -d "$SUPPORT_EXTENSIONS_PATH" ] && cp -vR "$SUPPORT_EXTENSIONS_PATH" "$EXTENSIONS_DIR"
